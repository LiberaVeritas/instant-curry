...
replace with `into`
*)
let rec _subst_expr from into within =
  let rec sub e =
    match e with
    | Nil -> if from = Nil then into else Nil
    | Cons (x, xs) -> Cons (sub x, sub xs)
    | ListCase (l, n, x, xs, c) -> 
      ListCase (sub l, sub n, x, xs, sub c)
    ...
    | Fun (f, ty, e) -> 
      if from = (Fun (f, ty, e)) then into else (Fun (f, ty, sub e))
    | MFun (f, ty, e) -> 
      if from = (MFun (f, ty, e)) then into else (MFun (f, ty, sub e))
...
(* if substituting variable, rename first *)
and subst_expr from into within =
  if from = within then into else
  (* prevent subbed functions from being evaluated *)
  let into =
  ...
  | BVar x -> 
    let x' = fresh x in
    let res = _subst_expr (BVar x) (BVar x') within in
    printf "AFTER rename\n";
    Core.Sexp.output_hum Stdio.stdout @@ sexp_of_tm res;
    printf "\n\n";
    _subst_expr (BVar x') into res
  | _ -> _subst_expr from into within
  in
  printf "AFTER subst\n";
    Core.Sexp.output_hum Stdio.stdout @@ sexp_of_tm x;
    printf "\n\n";
    x
...
let rec unrename e =
  ...
  | ListCase (l, n, x, xs, c) -> 
    ListCase (unrename l, unrename n, x, xs, unrename c)
  | Nat n -> Nat n
  ...
  | Fun (f, ty, e) -> Fun (f, ty, unrename e)
  | MFun (f, ty, e) -> MFun (f, ty, unrename e)
  | BVar v -> BVar (func v)
  | UVar v -> UVar (func v)
  | Ref v -> Ref (func v)
  | MRef v -> MRef (func v)
...
let rename_in_scope e =
  ...
  | ListCase (l, n, x, xs, c) -> 
    ...
    let xs' = fresh xs in
    let c' = 
      subst_expr (BVar x) (BVar x') c |> 
      subst_expr (BVar xs) (BVar xs')
    in
    ListCase (l, n, x', xs', c')
  | Nat n -> Nat n
  | Plus (e, e') -> Plus (e, e')
  ...
  | Fun (f, ty, e) -> 
    let f' = fresh f in
    let e' = subst_expr (BVar f) (BVar f') e in
    Fun (f', ty, e')
  | MFun (f, ty, e) -> 
    let f' = fresh f in
    let e' = subst_expr (BVar f) (BVar f') e in
...
(Core.Sexp.output_hum Stdio.stdout (sexp_of_tm e));
  ...
  | App (f, e') ->
    ...
    | Fun (x, _, body), _ -> 
      eval2 env @@ subst_expr (BVar x) right body 
      (*| App (_, _) -> let res = eval2 env f in
      begin match res with
      | App _ -> e
      | _ -> eval2 env @@ App (res, e')
      end *)
    | App (_, _), App (_, _) -> eval2 env @@ App (left, right)
...
let try_def_match env def prev curr =
  ...
  let rec go e next : bool =
    let res = restore @@ attempt e in
    let x = get_next next in
    
    printf "GET NEXT of sub 1 %s\n" fn;
    Core.Sexp.output_hum Stdio.stdout (sexp_of_tm x);
    printf "\n\n";
    printf "RES of sub 1 %s\n" fn;
    Core.Sexp.output_hum Stdio.stdout (sexp_of_tm res);
    printf "\n\n";
    (*printf "Candidate\n";
    Core.Sexp.output_hum Stdio.stdout (sexp_of_tm e);
  ...
  printf "\n\n";
    ...
    printf "\n\n";
    printf "EQUAL? %s\n" fn;
    Core.Sexp.output_hum Stdio.stdout (Core.sexp_of_bool (res = curr));
    printf "\n\n";*)
...
let check_def env defs prev curr =
  ...
  (*printf "\nLISTSILT\n";
  Core.Sexp.output_hum Stdio.stdout (Core.sexp_of_list sexp_of_tm defs);*)
...
let rename_in_scope_def d =
  let args' = List.map ~f:fun (a, ty) -> ((fresh a), ty) d.args in
    let body' = 
      List.fold2 ~f:fun acc (a, _) (a', _) -> 
...
let rename_in_scope_proof p =
  match p with 
  | Axiom -> p
  | Proof (var, c::cs) ->
    let var' = fresh var in
    let rename_in_scope_case c =
...
let rename_in_scope_thm t =
  


let rename_in_scope_stmt s =
  | Print tm -> Print tm
  | Definition d -> Definition @@ rename_in_scope_def d
  | Thm {name; stmt; proof} -> 
    printf "\nTheorem: %s\n" name;
    match proof with
    | Axiom -> 
    ...
    | Proof (n, c::cs) -> 
      ...
      (*let wtss = map (fun c -> infer_wts (fst cases) c stmt) (snd cases) in
      let cases = List.map2 (fun wts case -> inject_wts wts case) wtss cases
      in*)
...
let exec_stmt (env : env) (thms: thms) (s : stmt) : env * thms =
  match s with
  (*| Const (x, v) -> ()*)
  | Print tm -> print_endline @@ string_of_tm @@ eval2 env tm; (env, thms)
  | Definition d -> 
